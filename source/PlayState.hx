...
class PlayState extends FlxState {
	...
	function create_callback() {
		if (PlayState.l < 3)
			Chicken.age = 1;
		else if (PlayState.l < 6)
			Chicken.age = 2;
		else
			Chicken.age = 3;
		switch (PlayState.l) {
			case 1:
				Piggie.PATROL_MODE = Piggie.PATROL_MODE_PING_PONG_NEAREST;
			case 2, 5:
				Piggie.PATROL_MODE = Piggie.PATROL_MODE_LOOP_FORWARD_NEAREST;
			case 3, 4, 6, 7, 8:
				Piggie.PATROL_MODE = Piggie.PATROL_MODE_NEAREST;
		}
	}
	...
	function add_objects(_level_data:Array<Array<Int>>) {
		for (y in 0..._level_data.length) {
			for (x in 0..._level_data[y].length) {
				if (_level_data[y][x] == 2)
					add_chicken(x, y);
				if (_level_data[y][x] == 3)
					add_piggie(x, y);
				if (_level_data[y][x] >= 4)
					add_patrol_node(x, y, level_data[y][x] - 4);
				if (_level_data[y][x] == -1)
					add_egg(FlxPoint.get(x * 16 + 8, y * 16 + 8), false);
				if (_level_data[y][x] == -2)
					add_egg(FlxPoint.get(x * 16 + 8, y * 16 + 8), false, true);
			}
		}
		for (p in pig_queue) {
	...
	function check_objects(o1:GameSprite, o2:GameSprite) {
		if (o1.type == 'none' || o2.type == 'none' || o1.type == o2.type)
			return;
		if (o1.type == 'egg')
			o1.kill();
		if (o2.type == 'egg')
			o2.kill();
		if (o1.type == 'chicken' && o2.type == 'piggie' || o2.type == 'chicken' && o1.type == 'piggie')
			game_over();
	}
